...
function getsmparam($P_HANDLE, $P_TIMEOUT, $P_DEVADDR, $P_COMMAND) {
    ...
    if (!preg_match("/([0-9A-F]{2});FB;([0-9A-F]{2})/", $V_MSG, $matches)) {        flush();
        fclose($P_HANDLE);
        die("Invalid response from header");
    }

    if ($matches[1] != $P_DEVADDR) {        flush();
        fclose($P_HANDLE);
    ...
    if (!preg_match('/^\|64:(\w{3})=([0-9A-F]+)\|([0-9A-F]{4})}$/', $V_MSG, $matches)) {        flush();
        fclose($P_HANDLE);
        die("invalid response");
    }

    if ($matches[1] != $P_COMMAND['name']) {        flush();
        fclose($P_HANDLE);
        die("wrong response");
    }
...
function ping($host, $port, $timeout = 3) {    $fsock = fsockopen($host, $port, $errno, $errstr, $timeout);
    if (!$fsock) {        return 0;
    } else {        return $fsock;
    }
}
...
if ($V_SM_RESULT_COMMSTATE == 'ONLINE' && $V_SM_RESULT_AC_POWER_NOW < 50) {    $current_count = 0;
    #houston, we have a problem.  Log and monitor and email if more than three in a row.

    if (file_exists($logdata_file)) {        $count = file($logdata_file);
    ...
    if ($current_count >= 3) {        #then email me and let me know!
        ...
        foreach ($email_to_addresses as $email_to) {            mail($email_to, 'SOLAR PV ERROR', $body, $email_headers);
        }
        #reset the counter
...
if (!$response['liid'] || $response['liid'] == 0 || $response['liid'] == '') {    #format the array as a nice JSON string
    ...
    if ($response['errno'] != '1062') {        $json = json_encode($data);
        $fp = fopen($pvdata_file, 'a');
        fwrite($fp, $json . "\n");
        fclose($fp);
    }
} else {    #check to see if there is any data waiting to be written:
    if (file_exists($pvdata_file)) {        $handle = @fopen($pvdata_file, "r");
        ...
        if ($to_write == $responses) {            unlink($pvdata_file);
        } else {            $fp = fopen($error_file, 'a');
